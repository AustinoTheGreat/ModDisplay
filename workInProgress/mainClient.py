...
def on_connect(client, userdata, flags, rc):
    ...
    publish.single("RPi/Master", "type=pic/status=load", hostname = MQTT_BROKER)
...
def on_message(client, userdata, msg):
    ...
    nextStep = input(msg[0] + "Press c to continue, press q to quit everything, press n to load in new video or picture file: ")
    print(nextStep)
    if nextStep == "n":
    ...
    print(userMsg)
    if prev == "p":
        userMsg = userMsg[:userMsg.find("prev=")+5] + "p" + userMsg[userMsg.find("prev=")+6:]
    elif prev == "v":
        userMsg = userMsg[:userMsg.find("prev=")+5] + "v" + userMsg[userMsg.find("prev=")+6:]
    
    print(userMsg)

    if (float(prevTime) > float(curTime)):
        userMsg = userMsg.split("/")[0] + "/" + userMsg.split("/")[1] + "/" + userMsg.split("/")[2] + "/" + userMsg.split("/")[3] + "/" + prevTime
    else:
        userMsg = userMsg.split("/")[0] + "/" + userMsg.split("/")[1] + "/" + userMsg.split("/")[2] + "/" + userMsg.split("/")[3] + "/" + curTime
