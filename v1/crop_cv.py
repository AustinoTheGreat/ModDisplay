...
def crop(im, n_width, n_height, width, height):
    ...
    image2 = im[ int ((width - n_width)/ 2) : int (((width - n_width)/ 2) + n_width), int ((height - n_height)/2) : int (((height - n_height)/2) + n_height)]
    return image2
...
if (m1 == 0) and (m2 == 0):
    ...
    (height, width) = im2_new.shape[:2]
    img2 = im2_new[int (width/2): width, int(height/2) : height]
